...
CREATE TABLE concerts (
  ...
  price_customer int NOT NULL DEFAULT 0, -- Prix client
...
CREATE TABLE reservations (
  ...
  concert int unsigned NOT NULL,
  seats int NOT NULL DEFAULT 1,
  comment varchar(255) DEFAULT NULL,
  PRIMARY KEY (id),
  KEY reservations_owners (owner),
  KEY reservations_concert (concert),
  CONSTRAINT reservations_owners FOREIGN KEY (owner) REFERENCES visitors(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT reservations_concert FOREIGN KEY (concert) REFERENCES concerts(id) ON DELETE CASCADE ON UPDATE CASCADE
);
